...
在线接口：在调用在线接口时，钱包必须能够连接网络中的其他节点，而且能够正常同步区块和广播数据，且最好是已经同步到最新区块。接口所产生的数据都会保存在钱包中。例如创建账户、修改密码、转账、获取区块头等。

离线接口：对接钱包可单独运行在一个没有网络连接的节点上，无需同步区块。用户通过调用离线接口，传入相关的参数，获取返回值，相应数据不会存入钱包。例如离线创建账户、离线组装转账交易、离线签名等。
